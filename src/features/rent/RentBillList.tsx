...
export async function RentBillList({ platformId }: { platformId: string }) {
  const bills = await getRentBills(platformId)
  return (
    <div className="space-y-6">
      <RentBillForm platformId={platformId} />
      <Card title="Rent Bills">
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            ...
            {bills.length === 0 && (
              <tr>
                <td colSpan={8} className="py-4 text-center text-slate-400">No rent bills yet.</td>
              </tr>
            )}
          </tbody>
        </table>
